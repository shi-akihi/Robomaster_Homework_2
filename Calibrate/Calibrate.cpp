...
int main() {
    ...
    vector< Point2f > point_pix_pos_buf;//每张图片的角点
    vector< vector<Point2f> > point_pix_pos;//所有图片的角点
    int found, successes = 0;//successes：合法图片的数量
    Size img_size;
    int cnt = 0;
    int k = 0, n = 0;
    for (int i = 0; i <= 40; i++){
        cv::Mat src0 = cv::imread("chess/"+std::to_string(i)+".jpg");
        if ( !cnt ) {
        ...
        if ( found && point_pix_pos_buf.size() == board_n ) {//match
            ...
            point_pix_pos.push_back( point_pix_pos_buf );//存入图片的角点
            //draw
        ...
        point_pix_pos_buf.clear();//清空当前图片的角点
        cnt++;
    };
    ...
    vector< Point3f > point_grid_pos_buf;//当前图片角点在世界坐标系的坐标
    ...
    return 0;
}//g++ Calibrate.cpp -o Calibrate `pkg-config opencv4 --cflags --libs`